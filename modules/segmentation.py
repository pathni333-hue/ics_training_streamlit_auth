...
# Optional utility imports with safe fallback if not found
try:
    from utils import sample_network, score_segmentation
except ImportError:
    def sample_network(*args, **kwargs):
        st.warning("sample_network not available in utils.")
        return nx.DiGraph()
    def score_segmentation(*args, **kwargs):
        st.warning("score_segmentation not available in utils.")
        return 0
...
def draw_network_plotly(G):
    ...
    node_trace = go.Scatter(
        ...
        textposition='top center', marker=dict(size=sizes),
        text=list(G.nodes()), hovertext=text, hoverinfo='text'
    )
...
def app(user_context=None):
    ...
    if st.button('Calculate compliance score'):
        ...
        true_set, user_set = set(true_violations), set(user_violations)
